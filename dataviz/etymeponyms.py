...
df = pd.read_csv("datasets/etymeponyms.csv")
...
title = Image.from_column([
    Image.from_text("40 words named after historical figures".upper(), arial(48, bold=True), fg="white", bg="black")
...
grid = grid_chart(data, ignoring_exceptions(lambda d: get_non(d, "image", default_img)), image_process=process, padding=5, bg="black", title=title, yalign=0).pad((0,0,10,0), "black")
img = Image.from_column([grid, footer], bg="black", xalign=1, padding=5)
img.save("output/etymeponyms.png")
