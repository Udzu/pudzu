...
hos = tmap(RGBA, tmap(list, sns.light_palette((0.7,0.2,0.1))))
...
def colorfn(c):
    ...
    d = df.loc[c]
    if non(df['date'][c]) : return Stripe(20, "grey", hos[-1])
    y = int(df['date'][c]) // 10 - 196
    if non(df['hog'][c]): return hos[y]
    elif non(df['hos'][c]): return hog[y]
    elif non(df['hosdate'][c]): return Stripe(20, hog[y], both[y])
    else: return both[y]
    
chart = map_chart("maps/Europe2.png", colorfn, ignoring_exceptions(lambda c: str(int(df["date"][c]))), label_font=FONT(16, bold=True))

# generate legend
font_size = 16
box_size = 30
def box(c): return Image.new("RGBA", (box_size,box_size), c)
def stripebox(c1, c2): return Image.from_pattern(Stripe(20, c1, c2), (box_size,box_size))
...
note_leg = Image.from_text("*last 42 years only", FONT(font_size), bg="white")
...
n = 16
tdata = pd.DataFrame([[df.loc[i*n+j] for j in range(n) if i*n + j < len(df)] for i in range(3)])
...
# put it all together
title = Image.from_text("42 Years of Women Leaders in Europe".upper(), FONT(52, bold=True), "black", "white", padding=(0,20))
