...
def label(name, population):
    size = choose(population, [24, 20, 18])
...
lfooter = "\n• ".join(["Map is limited to places with 1000+ residents. Smaller locations include:\n", "Copenhagen NY (pop 801)", "Oslo MN (pop 330)", "Angora Township MN (pop 249)", "Kief ND (pop 13)", "Reykjavik MB (small community)", "Vatican LA (small community)", "Sofia NM (ghost town)", "Budapest GA (ghost town)"]) + "\n\nThere is also an Agram Township MN (pop 534), named after Zagreb's old German name."
...
title = Image.from_text("Capital cities of Europe".upper(), arial(60, bold=True))
