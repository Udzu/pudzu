...
df = pd.read_csv("datasets/flagsanimals.csv")
...
def process(d):
    ...
    return Image.from_column([
      Image.from_text(d['name'].replace(r'\n','\n').replace(', ','\n'), FONT(28, bold=True), align="center", beard_line=True, fg=fg, line_spacing=2),
      Image.from_text(description, FONT(24, italics=True), fg=fg, beard_line=True),
      flag
      ], padding=2, bg=bg, equal_widths=True)

title = Image.from_text("A few flags with animals on them".upper(), FONT(68, bold=True), fg=fg, bg=bg, align="center").pad(30, bg).pad((0,0,0,10), bg)
...
img.save("output/flagsanimals.png")
