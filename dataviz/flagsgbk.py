...
df = pd.read_csv("datasets/flagsgbk.csv")
...
def process(d):
    ...
    flag = flag.pad((1,1,0,1) if "Satakunta" in d['name'] else 1, "grey")
...
title = Image.from_text(f"Blue, gold and black flags".upper(), FONT(80, bold=True), fg=fg, bg=bg).pad(40, bg)
