...
df = pd.read_csv("datasets/flagsgwk.csv")
...
def process(d):
    ...
    flag = flag.pad((1,0,0,0) if "Mamluk" in d['name'] else 1, "grey")
...
title = Image.from_text(f"Green, white and black flags".upper(), FONT(80, bold=True), fg=fg, bg=bg).pad(40, bg)
