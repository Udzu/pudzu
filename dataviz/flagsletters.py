...
df = pd.read_csv("datasets/flagsletters.csv")
array = [[dict(r) for _,r in batch] for batch in generate_batches(df.iterrows(), 7)]
...
def process(d):
    ...
    flag = flag.pad((1,0,0,0) if "Ohio" in d['name'] else 1, "grey")
    return Image.from_column([
      Image.from_text_bounded(d['letter'].upper() + ". " + d['name'].replace(r"\n","\n"), (320 if "Switzerland" not in description else 200,200), 32, partial(FONT, bold=True), beard_line=True, align="center", fg=fg),
      Image.from_text(description, FONT(24, italics=True), fg=fg),
      flag
      ], padding=2, bg=bg, equal_widths=True)

title = Image.from_text(f"An alphabet of flags".upper(), FONT(96, bold=True), fg=fg, bg=bg).pad(40, bg)

grid = grid_chart(data, process, padding=(10,20), fg=fg, bg=bg, yalign=(0.5,1,0.5))
...
img.save("output/flagsletters.png")
