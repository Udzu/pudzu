...
df = pd.read_csv("datasets/flagsmono.csv")
array = [[dict(r) for _,r in rows] for rows in generate_batches(df.iterrows(), 4)]
...
def process(d):
    ...
    flag = flag.pad(1, "grey") if "Maratha" not in d['name'] else flag.pad((1,1,0,1), "grey")
    return Image.from_column([
      Image.from_text(d['name'], FONT(32, bold=True), beard_line=True, fg=fg),
      Image.from_text(description, FONT(24), fg=fg),
      flag
      ], padding=2, bg=bg, equal_widths=True)

title = Image.from_text("Monochromatic national flags".upper(), FONT(80, bold=True), fg=fg, bg=bg).pad(40, bg)
