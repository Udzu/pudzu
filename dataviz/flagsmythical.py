...
df = pd.read_csv("datasets/flagsmythical.csv")
groups = [g for g in remove_duplicates(df.group) if not g.startswith("#")]
...
def process(d):
    ...
    flag = flag.pad(0 if "coat" in d['group'] else (1,1,0,1) if "Venice" in str(d['description']) else 1, "grey")
...
title = Image.from_text(f"flags with mythical creatures".upper(), FONT(92, bold=True), fg=fg, bg=bg).pad(40, bg)
...
img.save("output/flagsmythical.png")
