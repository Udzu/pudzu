...
df = pd.read_csv("datasets/flagspolygons.csv")
...
def process(d):
    ...
    flag = flag.resize_fixed_aspect(height=198) if "square" in str(d['description']) else flag.resize((318,198))
...
title = Image.from_text(f"shapes on flags".upper(), FONT(80, bold=True), fg=fg, bg=bg).pad((0,30,0,10), bg)
subtitle = Image.from_text("(dates are of language creation, not flag adoption)", FONT(24, italics=False), fg=fg, bg=bg)
titles = Image.from_row([Image.from_column([title])], padding=(20,0), yalign=1)
...
img.save("output/flagspolygons.png")
