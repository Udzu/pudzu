...
df = pd.read_csv("datasets/flagspresident.csv")
...
def process(d):
    ...
    flag = flag.pad(1, "grey") if not any(x in d['name'] for x in ("Finland", "Iceland")) else flag.pad((1,1,0,1), "grey")
...
title = Image.from_text(f"A selection of Presidential standards".upper(), FONT(80, bold=True), fg=fg, bg=bg).pad(40, bg)
...
img.save("output/flagspresident.png")
