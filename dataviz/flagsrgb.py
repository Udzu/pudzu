...
df = pd.read_csv("datasets/flagsrgb.csv")
...
def process(d):
    ...
    flag = flag.pad(0 if "coat" in d['group'] else (1,0,0,0) if "Tunisia" in str(d['name']) else 1, "grey")
...
title = Image.from_text(f"Red, green and blue flags".upper(), FONT(80, bold=True), fg=fg, bg=bg).pad(40, bg)
