...
df = pd.read_csv("datasets/flagsswastika.csv")
...
def process(d):
    ...
    flag = flag.pad(1 if "Pres" not in d["name"] else (1,1,0,1), "grey") 
...
title = Image.from_text("Flags containing swastikas".upper(), FONT(76, bold=True), fg=fg, bg=bg).pad(40, bg)

grid = grid_chart(data, process, padding=(10,20), fg=fg, bg=bg, yalign=(0.5,1,0.5), row_label=lambda r: None if data.index[r].startswith("_") else Image.from_text(data.index[r].replace(r"\n","\n").upper(), FONT(32, bold=True), align="center"))
...
img.save("output/flagsswastika.png")
