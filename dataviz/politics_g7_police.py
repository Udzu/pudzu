...
guns = bar_chart(df[["guns_ph"]], 40, 350, horizontal=True, spacing=2, label_font=FONT(16), rlabels=rlabel,
...
homicides = bar_chart(df[["homicide_pm"]], 40, 350, horizontal=True, spacing=2, label_font=FONT(16), rlabels=rlabel,
                  ...
                  grid_interval=5, ymax=40, ylabel=Image.from_text("intentional homicides per million population (2015)", FONT(18), padding=5))

df = df.sort_values("policedeaths_pm", ascending=False)
policedeaths = bar_chart(df[["policedeaths_pm"]], 40, 850, horizontal=True, spacing=2, label_font=FONT(16), rlabels=rlabel,
                  ...
                  grid_interval=0.25, label_interval=0.5, ymax=3.5, ylabel=Image.from_multitext(["killings by law enforcement officers per million population (2015) ", "(unofficial estimates apart from UK and Germany)"], [FONT(18), FONT(14)]).pad(5,0))
                  
chart = Image.from_column([policedeaths, Image.from_row([guns, homicides])], bg="white", padding=10)
title = Image.from_column([Image.from_text("Police killing rates in G7 countries".upper(), FONT(40, bold=True)), Image.from_text("compared to gun ownership and homicide rates", FONT(36))])
footer = Image.from_text("Police killing estimates from a variety of sources. Gun ownership and homicide data from Wikipedia.", FONT(16), padding=5)
