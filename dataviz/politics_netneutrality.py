...
img = bar_chart(combined, 80, 800, spacing=5, type=BarChartType.STACKED, colors=(RCOL, DCOL, ICOL), ymax=250,
    grid_interval=50, tick_interval=25, label_font=arial(FONT_SIZE), clabels=lambda c,r,v,w,h: None if v < 3 else Image.from_text_bounded(str(int(v)), (w,h), FONT_SIZE, papply(arial, bold=True), fg="white", padding=(0,0,0,1)),
