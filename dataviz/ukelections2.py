...
SHORT_NAMES = { "Liberal Democrats": "Lib Dem", "Conservatives": "Tory" }
...
def clabel_pc(c, r, below=False):
    label = f"{df.votes_pc[r]:.1%}\n{df.seats[r]} seat" + "s"*(df.seats[r]>1)
    display = (abs(df.leaning[r]) < 0.1) == below
...
ylabel = Image.from_text("level of (under-/over-)representation, based on odds ratio of votes to seats", FONT(24), padding=5, bg="white").transpose(Image.ROTATE_90)
...
bar = bar_chart(df[["leaning"]], 62, 1000, spacing=2, colors=color_fn, ymin=-1, ymax=1, grid_interval=0.125, 
    label_font=FONT(12), rlabels=None, clabels={
         ...
         BarChartLabelPosition.INSIDE : clabel_pc,
...
title = Image.from_column([
    ...
    , Image.from_text("vote share versus seat share in UK Elections (1979-2017)".upper(), FONT(36), bg="white")
