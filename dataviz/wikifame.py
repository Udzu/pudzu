...
SPECS = [["datasets/wikibirths.csv", "output/wikibirths.jpg",
         ...
         ["datasets/wikibirths_normalised.csv", "output/wikibirths_normalised.jpg",
          "famous people from the second millennium", "the most famous person born each decade, according to Wikipedia",
          "{}00s", "'{}0s", range(10,19), range(0,10)]
          ]
...
for DATASET, OUTPUT, TITLE, SUBTITLE, ROWFORMAT, COLFORMAT, ROWRANGE, COLRANGE in SPECS[-1:]:
    ...
    def process(name):
        ...
        box = box.place(Image.from_column([
          Image.from_url_with_cache(get_non(df['image_url'], name, DEFAULT_IMG)).crop_to_aspect(100, 100, (0.5, 0.2)).resize_fixed_aspect(width=160),
    ...
    grid = grid_chart(table, process, row_label=arial(20, bold=True), col_label=arial(20, bold=True), bg=bg)
