...
def entitle(img):
    ...
    img = img.pad((0,0,0,20), bg).place(Image.from_text("/u/Udzu", FONT(16), fg=fg, bg=bg, padding=5).pad((1,1,0,0), fg), align=1, padding=10)
    return img
...
chart = entitle(chart)
chart.convert("RGB").save("output/wikifame_eujews.jpg")
