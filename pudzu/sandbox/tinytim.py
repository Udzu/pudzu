...
class LSystem:
    ...
    def plot(self, iterations, size, reset=True, tracer=(0,0)):
        if reset: wn.clearscreen()
        wn.tracer(*tracer)
        stack = []
        for c in self.expand(iterations):
            if c == "F": fd(size)
            elif c == "G": pu(); fd(size); pd()
            elif c == "+": rt(self.angle)
            elif c == "-": lt(self.angle)
            elif c == "[": stack.append((position(), heading()))
            elif c == "]": p, h = stack.pop(); pu(); setposition(p); setheading(h); pd()
        wn.update()
